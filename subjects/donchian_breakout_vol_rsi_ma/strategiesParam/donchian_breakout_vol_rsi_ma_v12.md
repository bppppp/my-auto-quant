...
- name: partial_profit_pct
  ...
  reason: v11 overbought_reduce胜率100%均收+5801,5份报告稳定,保持0.15
- name: rebalance_freq_days
  ...
  reason: 组合层参数,5份报告无频率问题,5日与中周期波段匹配,保持
- name: rsi_entry_high
  default: 80
  range:
  - 50
  ...
  reason: 入场0触发为结构问题(close>donchian_high_20不成立),参数非主因,保持80
- name: atr_stop_multiplier
  ...
  reason: v11触发11334次胜率3.95%,历史测试3.0最优,继续上调均收反恶化,保持
- name: trail_stop_pct
  default: 0.08
  range:
  - 0.05
  ...
  reason: v11 0.14过松,触发仅1474次P/L=1.41,收紧至0.08恢复profit保护,兼顾P/L
- name: vol_breakout_threshold
  ...
  reason: 入场0触发为结构问题(close>donchian_high_20),0.7已较松,保持
...
- name: target_holdings
  ...
  reason: 8只×10%=80%资金利用率,5份报告稳定,保持
- name: rsi_overbought
  default: 75
  range:
  - 65
  ...
  reason: v11 overbought_reduce胜率100%均收+5801,5份报告稳定,保持75
- name: fixed_stop_loss_pct
  ...
  reason: v11触发3034次为5份最高,12%贴合中周期,trail收紧后预期降,保持
- name: add_position_weight_threshold
  ...
  reason: 入场未生效,加仓逻辑未触发,保持0.3待入场结构修复后验证
...
- name: rsi_entry_low
  ...
  reason: 入场0触发为结构问题,20放宽窗口保留动量机会,保持
- name: reduce_position_floor
  ...
  reason: 分级减仓未启用,floor暂不生效,保持0.03
- name: max_holding_days
  ...
  reason: time_stop胜率72.3%均收+1040,5份报告稳定,保持30
- name: reduce_position_weight_threshold
  ...
  reason: 分级减仓未启用,参数待入场逻辑修复后验证,保持0.3
- name: max_single_weight
  ...
  reason: 与target_holdings=8匹配,集中度非核心问题,保持0.1
