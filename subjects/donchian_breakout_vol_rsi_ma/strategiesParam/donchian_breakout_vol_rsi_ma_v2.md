...
- name: partial_profit_pct
  ...
  reason: 本轮不变；与rsi_overbought联动表现优异(均收+5737)，维持原值
- name: rebalance_freq_days
  ...
  reason: 本轮不变；5日频率与中周期波段匹配，不影响入场问题根因
- name: rsi_entry_high
  default: 65
  range:
  - 50
  ...
  reason: rsi_14 p75=62.3，当前70过高阻挡大量入场，下调至65匹配分位数
- name: atr_stop_multiplier
  ...
  reason: 本轮不变；触发9681次胜率10.65%偏低，待入场恢复后下一轮再评估
- name: trail_stop_pct
  default: 0.08
  range:
  - 0.03
  ...
  reason: 20089次触发胜率仅22.55%，止损过紧反复打掉趋势，放宽至0.08
- name: vol_breakout_threshold
  ...
  reason: volume_ratio_20 p75=1.19，当前1.5远高于此导致入场信号为0，下调至1.0
- name: max_industry_concentration
  ...
  reason: 本轮不变；组合层参数，不影响入场/出场核心问题
- name: target_holdings
  ...
  reason: 本轮不变；先解决入场问题，持仓数与max_single_weight=0.1匹配合理
- name: rsi_overbought
  default: 75
  range:
  - 65
  ...
  reason: 本轮不变；触发4141次胜率100%均收+5737，最佳出场信号保持
- name: fixed_stop_loss_pct
  ...
  reason: 3202次触发均损-2883，647次涨跌停吞单，放宽至0.10减少假止损
- name: add_position_weight_threshold
  ...
  reason: 本轮不变；当前无入场，加仓阈值无影响，待入场恢复后下一轮评估
- name: max_turnover_per_rebalance
  ...
  reason: 本轮不变；换手率与入场恢复程度相关，待下一轮评估
- name: rsi_entry_low
  default: 30
  range:
  - 20
  ...
  reason: rsi_14 p25=37.9，当前40接近下沿；联动breakout下调后，放宽至30扩大入场窗口
- name: reduce_position_floor
  ...
  reason: 本轮不变；当前为清仓式出场，floor未生效，待分级减仓启用后再调
- name: max_holding_days
  ...
  reason: 本轮不变；触发11578次胜率86.41%均收+1443，表现良好保留
- name: reduce_position_weight_threshold
  ...
  reason: 本轮不变；出场信号均直接清仓未触发分级减仓，参数待验证
- name: max_single_weight
  ...
  reason: 本轮不变；与target_holdings=8匹配，年化6.29%不源于集中度问题
