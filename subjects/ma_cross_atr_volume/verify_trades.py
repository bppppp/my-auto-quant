...
def main():
    ...
    for n, t in enumerate(all_trades, 1):
        ...
        if len(sub) < 20:
            print(f"    [FAIL] history < 20")
        else:
            f = strategy.compute_factors(sub, params)
            fv = {k: float(v.iloc[-1]) for k, v in f.items()}
            print(f"    factors: close={fv['close']:.3f}  ma_5={fv['ma_5']:.3f}  ma_20={fv['ma_20']:.3f}"
            ...
            cb = can_buy(bar)
            print(f"    A-share: can_buy={cb}  (ST={bool(bar.get('是否ST', False))}, "
                  f"limit_up={is_limit_up(bar)}, one_word_zt={is_one_word_board(bar) and is_limit_up(bar)}, "
                  f"raw_is_zt={bar.get('是否涨停')}, pct={bar.get('涨幅%')}%)")
            if not cb:
                print(f"    -> [FAIL] A-share rule violated at entry!")
        print()

        # === EXIT verification ===
        print(f"  [EXIT @ {xd}  signal={t['exit_signal']}]")
        ...
        if len(sub) < 20:
            ...
            if sig == "ma_death_cross":
                ...
                ok = fv["close"] < thr
                detail = f"{fv['close']:.3f} < {thr:.3f}  (entry={t['entry_price']:.3f} * {1-params['fixed_stop_pct']:.3f})"
            elif sig == "time_stop":
            ...
            cs = can_sell(bar)
            print(f"    A-share: can_sell={cs}  (limit_down={is_limit_down(bar)}, "
                  f"one_word_dt={is_one_word_down(bar)}, pct={bar.get('涨幅%')}%)")
            if not cs:
