...
# command: python generated/strategy.py weight --weight-test multi_factor_trend_swing   ← 覆盖默认 (一般不需要)
# command: python generated/strategy.py --start-date 2024-01-01 --end-date 2024-12-31

"""multi_factor_trend_swing 策略. 由 LLM 从 _original.md 翻译.
...
**关键设计**:
- 策略名隐含在文件路径中 (本文件即 `multi_factor_trend_swing` 策略), 无需 `--strategy`
...
**与 ma_cross_atr_volume / donchian_breakout_vol_rsi_ma 的差异**:
- 本策略**不进行加仓/减仓** (narrative §4 显式说明, params 段无 add/reduce_position_weight_threshold 字段),
  仓位由 entry_score 排名 + `target_holdings` 决定.
- 入场采用 5 因子**等权共振** (0.35+0.20+0.15+0.15+0.15), 与 ma_cross 的"金叉必触发 + 至少 1 辅助"
  和 donchian 的"多信号 Σ(weight)"均不同 — 这里是单纯多信号加权和, 没有硬性必触发条件.
"""
...
class Strategy:
    ...
    def entry_score(self, factors: dict, params: dict, weights: dict) -> float:
        # spec narrative §4: 多因子加权机制, 综合得分 = Σ(触发信号的权重)
        # 5 个信号相互独立, 任一不触发就少加一项; runner 侧按 score 降序选 top N.
        score = 0.0
        ew = weights["entry"]
        # trend_strength (weight 0.35): ma_10 > ma_30 AND ma_30 > ma_60  (均线多头排列)
        if (
            factors["ma_10"].iloc[-1] > factors["ma_30"].iloc[-1]
            and factors["ma_30"].iloc[-1] > factors["ma_60"].iloc[-1]
        ):
            score += ew["trend_strength"]
        # atr_filter (weight 0.20): atr_14 / close > {atr_threshold}
        if factors["atr_14"].iloc[-1] / factors["close"].iloc[-1] > params["atr_threshold"]:
            score += ew["atr_filter"]
        # volume_confirm (weight 0.15): volume_ratio_20 > {vol_threshold}
        if factors["volume_ratio_20"].iloc[-1] > params["vol_threshold"]:
            score += ew["volume_confirm"]
        # momentum_filter (weight 0.15): mom_60 > {mom_threshold}
        if factors["mom_60"].iloc[-1] > params["mom_threshold"]:
            score += ew["momentum_filter"]
        # rsi_filter (weight 0.15): rsi_14 < {rsi_upper}  (避免超买)
    ...
    def should_exit(self, position: dict, factors: dict, params: dict, weights: dict) -> str | None:
        ...
        #   fixed_stop(0.30) = trailing_stop(0.30) > trend_reversal(0.20) > time_stop(0.10) = rsi_overbought(0.10)
    ...
    # NOTE: 本策略**不进行加仓/减仓** (narrative §4 显式说明).
    #   spec.params 段无 add_position_weight_threshold / reduce_position_weight_threshold /
    #   reduce_position_floor 字段, 仓位由 entry_score 排名 + params["target_holdings"] 决定.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="strategy.py",
        description="multi_factor_trend_swing 策略回测入口 (策略名隐含在文件路径中)",
    ...
    def run_once() -> int:
        ...
        eff_weight_test = args.weight_test if args.weight_test else "multi_factor_trend_swing"
        ...
        cli_args = ["run", "--strategy", "multi_factor_trend_swing", "--mode", args.mode]
