...
| **3. 牛 / 熊 / 震荡 3 环境处理** | 3 环境差异化处理（所有阈值 param 化） | 部分（`bear_*` params） |
| **4. 多信号逻辑关系** | 入场时机 + 出场优先级 | 无 |
| **5. 风险机制** | 熊市风控 + 涨跌停挤不出场 + 优先级链 + **早期数据 NaN 处理** | 无 |
| **6. 与其他策略区别** | 3 行以内差异化定位 | 无 |
...
1. **熊市风控**——降低仓位 + 收紧门槛
2. **涨跌停挤不出场**——信号被吞 / 不静默 / 不算反事实收益
3. **早期数据 NaN 处理**（**A 股硬约束**）——策略 narrative **必须**显式说明：

   | 场景 | 问题 | 必须说明的处理方式 |
   |---|---|---|
   | **上市未满 N 日** | ma_60 / kdj_9 / atr_14 / donchian_20 等 N 日窗口因子值为 NaN | 该股票**该日不参与信号计算**（不剔除，避免流动性问题）|
   | **长期停牌** | 因子值为 NaN 或停滞 | **复牌当日不立即入场**（等 N 日回填后再参与）|
   | **涨跌停日** | `true_range` / `volume` 计算可能异常 | **出场信号被吞**；**入场信号正常生效**（开盘时判断） |
   | **一进 / 退市** | 上市新股 / 退市股 | **默认跳过**（不参与回测，由 backtest 层处理）|

4. **优先级链**——多信号触发时按权重 / 优先级决定出场顺序

> **A 股硬约束（涨跌停 / 停牌 / 复牌 / 一字板默认跳过）由 backtest 代码生成器在生成回测代码时强制兑现，strategy_narrative 第 5 节仅说明"业务规则"，不写实际 backtest 代码。
...
5. **结构完整性**（frontmatter 7 区块 + strategy_narrative ≥ 1500 字符 / 信号结构化 / 止损 ≥ 3 类）
...
- ❌ **`strategy_narrative` 字符数 < 1500**（过短不通过）
- ❌ **`strategy_narrative` 缺少 6 节中任一节**（思路 / 环境 / 3 环境处理 / 多信号关系 / 风险机制 / 差异化）
- ❌ **`strategy_narrative` 第 5 节缺早期数据 NaN 处理**（未说明上市未满 N 日 / 停牌 / 涨跌停 / 一字板 4 类场景的处理 = 不通过）
- ❌ **`strategy_narrative` 第 3 节缺 3 环境处理**（牛 / 熊 / 震荡必须全有）
...
### `### 1.` ~ `### 6.` 字面要求（避免 #15 硬校验失败）

`strategy_narrative` 字段必须**逐字**包含以下 6 个 markdown 三级标题（**阿拉伯数字 + 英文句点**；不要用中文数字 `一、二、` 或顿号 `1、`）：

```
### 1.   ### 2.   ### 3.   ### 4.   ### 5.   ### 6.
```

6 节内容依次为：**策略思路** → **市场环境假设** → **牛/熊/震荡 3 环境处理**（三种全写）→ **多信号逻辑关系** → **风险机制**（必含 A 股 4 类 NaN 处理：上市未满 N 日 / 停牌 / 涨跌停 / 一字板）→ **与其他策略区别**（3 行内）。
